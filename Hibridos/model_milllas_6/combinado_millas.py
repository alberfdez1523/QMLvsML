...
CQ_MODEL_PATH = 'model_cq_miles.pt'
QC_MODEL_PATH = 'model_qc_miles.pt'
CLASSICAL_MODEL_PATH = 'model_classical_miles.pt'
...
class ClassicalModel(nn.Module):
    def __init__(self):
        ...
        # Initialize with random weights - the model should learn km to miles conversion
...
def generate_distance_data(n_samples, input_features=1):
    # Generate random kilometers between 0 and 200
    kilometers = np.random.uniform(0, 200, (n_samples, 1))
    
    # Convert to miles: miles = km * 0.621371
    ...
    if input_features == 1:
        ...
        # Add random noise features (could be road condition, incline, etc.)
    ...
    return torch.tensor(X_normalized, dtype=torch.float32), torch.tensor(miles, dtype=torch.float32)
...
def train_and_save_models(force_retrain=False):
    ...
    if (not force_retrain and 
        ...
        X_train, y_train = generate_distance_data(100, input_features=input_size)
        ...
        # Use Adam optimizer for better convergence
        optimizer_classical = optim.Adam(model_classical.parameters(), lr=1)
        
        # Add learning rate scheduler with verbose reporting
        scheduler_classical = optim.lr_scheduler.ReduceLROnPlateau(optimizer_classical, 'min', patience=100, factor=0.5)
...
def evaluate_models(model_cq, model_qc, model_classical, input_size):
    ...
    # Generate test data
    X_test, y_test = generate_distance_data(1000, input_features=input_size)
    ...
    with torch.no_grad():
        ...
        # Define accuracy as predictions within ±threshold miles of true value
        threshold = 0.2
        ...
        kilometers_test = X_test[:, 0].numpy() * 200
        ...
        plt.xlabel('Prediction Error (miles)')
        plt.ylabel('Count')
        ...
        plt.subplot(2, 2, 3)
        plt.hist(kilometers_test, bins=20, alpha=0.5, label='Kilometers', color='blue')
        plt.hist(y_test_np, bins=20, alpha=0.5, label='Miles', color='red')
        plt.xlabel('Distance')
        plt.ylabel('Count')
        ...
        plt.ylabel('Accuracy (%)')
        plt.title(f'Model Accuracy (±{threshold} miles)')
        for i, v in enumerate(heights):
            plt.text(i, v+1, f"{v:.2f}%", ha='center')
        plt.grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        plt.savefig('distance_conversion_comparison.png')
        plt.show()
        ...
        for i in range(min(20, len(y_test_np))):
            km_dist = round(float(kilometers_test[i]), 1)
            true_mi = round(float(y_test_np[i]), 2)
            ...
            cq_accurate = "Yes" if abs(true_mi - cq_pred) < threshold else "No"
            qc_accurate = "Yes" if abs(true_mi - qc_pred) < threshold else "No"
            classical_accurate = "Yes" if abs(true_mi - classical_pred) < threshold else "No"
            
            print("{:<5} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8} | {:<12} | {:<12} | {:<12}".format(
                i, km_dist, true_mi, cq_pred, qc_pred, classical_pred, cq_accurate, qc_accurate, classical_accurate))
