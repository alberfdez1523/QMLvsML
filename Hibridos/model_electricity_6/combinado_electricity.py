...
CQ_MODEL_PATH = 'model_cq_electricity.pt'
QC_MODEL_PATH = 'model_qc_electricity.pt'
CLASSICAL_MODEL_PATH = 'model_classical_electricity.pt'
...
class ClassicalModel(nn.Module):
    def __init__(self):
        ...
        # Initialize with random weights - the model should learn electricity bill calculation
        self.layer1 = nn.Linear(1, 1)
        
    def forward(self, x):
        return self.layer1(x)

# Generate electricity bill data using the formula: C = m * kWh + b
def generate_electricity_data(n_samples, input_features=1, m=0.15, b=10):
    # Generate random kWh values between 0 and 1000
    kwh = np.random.uniform(0, 1000, (n_samples, 1))
    
    # Calculate bill cost using formula: C = m * kWh + b
    bill_cost = m * kwh + b
    
    # Create feature vector - include kWh and other features if needed
    if input_features == 1:
        X = kwh
    else:
        # Add additional random features for testing model capabilities
        X = np.zeros((n_samples, input_features))
        X[:, 0] = kwh.flatten()  # First feature is actual kWh usage
        # Add random noise features (could be time of day, season, etc.)
    ...
    # Also normalize bill costs to be in a similar range
    bill_cost_normalized = bill_cost / 200.0  # Assuming max bill around $200
    
    return torch.tensor(X_normalized, dtype=torch.float32), torch.tensor(bill_cost_normalized, dtype=torch.float32)

def train_and_save_models(force_retrain=False):
    """Train models and save them, or load if they already exist."""
    # Common parameters for all models
    input_size = 8  # kWh + additional features
    ...
    if (not force_retrain and 
        ...
        X_train, y_train = generate_electricity_data(100, input_features=input_size)
        ...
        scheduler_cq = optim.lr_scheduler.ReduceLROnPlateau(optimizer_cq, 'min', patience=10, factor=0.5)
        ...
        # Add learning rate scheduler
        scheduler_qc = optim.lr_scheduler.ReduceLROnPlateau(optimizer_qc, 'min', patience=10, factor=0.5)
        ...
        for epoch in range(epochs):  
            # Forward pass - use only the first feature (kWh)
...
def evaluate_models(model_cq, model_qc, model_classical, input_size):
    ...
    # Generate test data
    X_test, y_test = generate_electricity_data(1000, input_features=input_size)
    ...
    with torch.no_grad():
        ...
        # Define accuracy as predictions within ±threshold of true value
        # For bill prediction, we'll use a relative threshold (e.g., 2% of the normalized range)
        threshold = 0.001
        ...
        # Classical model - use only kWh feature
        ...
        # Get the original kWh values (denormalize)
        kwh_test = X_test[:, 0].numpy() * 1000
        
        # Denormalize bill costs for better visualization (multiply by 200)
        y_test_np_denorm = y_test_np * 200
        y_pred_cq_np_denorm = y_pred_cq_np * 200
        y_pred_qc_np_denorm = y_pred_qc_np * 200
        y_pred_classical_np_denorm = y_pred_classical_np * 200
        
        # Create figure with multiple subplots
        plt.figure(figsize=(15, 10))
        
        # Plot 1: Actual vs Predicted Bill Costs
        plt.subplot(2, 2, 1)
        plt.scatter(kwh_test, y_test_np_denorm, color='black', label='True', marker='o')
        plt.scatter(kwh_test, y_pred_cq_np_denorm, color='blue', label='CQ Model', marker='x')
        plt.scatter(kwh_test, y_pred_qc_np_denorm, color='red', label='QC Model', marker='+')
        plt.scatter(kwh_test, y_pred_classical_np_denorm, color='green', label='Classical', marker='*')
        plt.xlabel('Electricity Usage (kWh)')
        plt.ylabel('Bill Cost ($)')
        plt.title('Electricity Bill: True vs Predicted')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # Plot 2: Prediction Error Distribution
        plt.subplot(2, 2, 2)
        errors_cq = y_pred_cq_np_denorm - y_test_np_denorm
        errors_qc = y_pred_qc_np_denorm - y_test_np_denorm
        errors_classical = y_pred_classical_np_denorm - y_test_np_denorm
        ...
        plt.subplot(2, 2, 3)
        plt.hist(kwh_test, bins=20, alpha=0.5, label='kWh Usage', color='black')
        plt.hist(y_test_np_denorm, bins=20, alpha=0.5, label='Bill Cost', color='blue')
        plt.xlabel('Value')
        plt.ylabel('Count')
        plt.title('Distribution of kWh Usage and Bill Cost')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.ylabel('Accuracy (%)')
        plt.title(f'Model Accuracy (±{threshold*100:.0f}% margin)')
        for i, v in enumerate(heights):
            plt.text(i, v+1, f"{v:.2f}%", ha='center')
        plt.grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        plt.savefig('electricity_bill_comparison.png')
        plt.show()
        
        # Print detailed prediction comparison for a few samples
        print("\nDetailed Electricity Bill Prediction Comparison:")
        print("{:<5} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8} | {:<12} | {:<12} | {:<12}".format(
            "Index", "kWh", "True $", "CQ Pred", "QC Pred", "Classical", "CQ Acc?", "QC Acc?", "Class Acc?"))
        print("-" * 105)
        
        for i in range(min(20, len(y_test_np))):
            kwh = round(float(kwh_test[i]), 1)
            true_bill = round(float(y_test_np_denorm[i]), 2)
            cq_pred = round(float(y_pred_cq_np_denorm[i]), 2)
            qc_pred = round(float(y_pred_qc_np_denorm[i]), 2)
            classical_pred = round(float(y_pred_classical_np_denorm[i]), 2)
            
            # Using the original threshold on normalized values
            cq_accurate = "Yes" if abs(y_test_np[i] - y_pred_cq_np[i]) < threshold else "No"
            qc_accurate = "Yes" if abs(y_test_np[i] - y_pred_qc_np[i]) < threshold else "No"
            classical_accurate = "Yes" if abs(y_test_np[i] - y_pred_classical_np[i]) < threshold else "No"
            
            print("{:<5} | {:<8} | {:<8} | {:<8} | {:<8} | {:<8} | {:<12} | {:<12} | {:<12}".format(
                i, kwh, true_bill, cq_pred, qc_pred, classical_pred, cq_accurate, qc_accurate, classical_accurate))
        ...
        threshold_dollar = threshold * 200  # Convert normalized threshold to dollars
        print(f"\nCQ Model: {cq_accurate_count}/{len(y_test_np)} predictions within +${threshold_dollar:.2f} margin ({accuracy_cq.item()*100:.2f}%)")
        print(f"QC Model: {qc_accurate_count}/{len(y_test_np)} predictions within +${threshold_dollar:.2f} margin ({accuracy_qc.item()*100:.2f}%)")
        print(f"Classical Model: {classical_accurate_count}/{len(y_test_np)} predictions within +${threshold_dollar:.2f} margin ({accuracy_classical.item()*100:.2f}%)")
    
        # Show exact conversion formula for reference
        print("\nReference: Cost = 0.15 * kWh + 10")
        
        # Calculate mean costs as a sanity check
        mean_kwh = np.mean(kwh_test)
        expected_bill = 0.15 * mean_kwh + 10
        print(f"Sanity check: For mean usage of {mean_kwh:.2f} kWh, expected bill is ${expected_bill:.2f}")
        print(f"Mean true bill in test data: ${np.mean(y_test_np_denorm):.2f}")
